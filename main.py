...
def draw_example_6(variable):
    turtle = Turtle()
    turtle.left(90)
    turtle.speed("fastest")
    turtle.getscreen().tracer(0, 0)
    turtle.up()
    turtle.setposition(238, -238)
    turtle.down()
...
def main():
    examples = [("A", [("A", "AB"), ("B", "A")]),
                ...
                ("Y", [("X", "X[-FFF][+FFF]FX"), ("Y", "YFX[+Y][-Y]")], draw_example_5),
                ("FF+FF+FF+FF", [("F", "F+F-F-F+F")], draw_example_6)]
